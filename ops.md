...
2. If uplink_alive is false: run bash /workspace/desk/bin/uplink.sh restart, then re-run ops-check.sh once. (Since 2026-09-08 the uplink runs under uplink-supervisor.sh, which restarts it by itself within 5 s of any exit and after code updates; supervisor_alive in the JSON says whether that loop is up. You are the fallback for what it cannot cover: a fresh computer after "Update Agent Computer", or both processes gone.)
